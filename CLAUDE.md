...
| **the flagship** | `make flagship` — builds examples/water, examples/flow, examples/bgk, examples/xy-gpu, examples/ising and examples/cloth (need g++-16); NO runner reaches them, so this is the local gate that keeps the door from rotting. `bgk` is the 3D one: tensor's BGK example reproduced in the file, 8192 particles device-resident through `Sync<Tensor<f32,N,3>>` — the positions the vertex shader reads are the buffers tensor evaluated into |
